...
#         train_pkl_file = open(video_pkl_file, 'rb')
        self.data = os.listdir(video_pkl_file)
        self.data.sort()
        self.curr = 1
...
#         print('index',index)
#         for img_name in range(self.curr,index):
        for img_name in range(index,index+1):
            img = Image.open(self.video_pkl_file+self.data[index])
...
class ImageTestDataset(Dataset):
    ...
    def __getitem__(self, index):
        frame_seq = []
#         print('index'.index)
        for img_name in range(self.nt):
            img = Image.open(self.video_pkl_file+self.data[img_name])
            img = self.transform(img)
            frame_seq.append(img)
        frame_seq = torch.stack(frame_seq, 0)
        return frame_seq
